...
enum { wbMPI_timerTag = 2, wbMPI_loggerTag = 4, wbMPI_solutionExistsTag  = 8, wbMPI_solutionTag = 16 };
	  ...
	  if (isMasterQ) {
	    ...
	    cout << "{\n";
		    ...
		    cout << wbString_quote("solutionExists") << ":";
				...
				const char *msg = wbMPI_getStringFromRank(ii, wbMPI_solutionExistsTag);
				if (msg != NULL) {
				  cout << msg;
				  //		 free(msg);
				}
	    ...
	    if (solutionJSON) {
	      ...
	      wbMPI_sendStringToMaster(solExistsString.c_str(), wbMPI_solutionExistsTag);
